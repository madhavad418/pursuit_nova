...
export function ProspectEditor({ leadId, open, onClose, onSaved, onToast }) {
    const [detail, setDetail] = useState(null); const [form, setForm] = useState(null); const [users, setUsers] = useState([]); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [coOwners, setCoOwners] = useState([])
    useEffect(() => {
    ...
    const canSave = detail && (allKeys.some(k => !locked(k)) || perms.can_manage_owners)
    const save = async e => {
        ...
        // Additional owners: the list without whoever is the primary owner, sent only when it differs from what was loaded
        if (perms.can_manage_owners) {
            const next = coOwners.filter(id => String(id) !== form.owner_id); const before = (detail.lead.co_owners || []).map(p => p.id).filter(id => String(id) !== form.owner_id)
            if (next.length !== before.length || next.some(id => !before.includes(id))) leadChanges.co_owner_ids = next
        }
    ...
    return <Modal open={open} onClose={onClose} title="Edit prospect" eyebrow={detail?.lead.company_name || 'Loading…'} size="xl">
        ...
        {form && <form onSubmit={save}>
            ...
            <div className="form-section"><h3>Prospect ownership</h3><div className="form-grid">
                ...
                {(perms.can_manage_owners || coOwners.length > 0) && <Field label="Additional owners" hint={perms.can_manage_owners ? 'Add as many people as needed — each sees and can work on this prospect' : 'Set by an Admin'} className="span-2"><PeoplePicker disabled={!perms.can_manage_owners} users={users} known={detail?.lead.co_owners || []} exclude={form.owner_id} value={coOwners.filter(id => String(id) !== form.owner_id)} onChange={setCoOwners} /></Field>}
