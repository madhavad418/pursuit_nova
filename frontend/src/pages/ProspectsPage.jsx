...
const STATUSES=['New','Assigned','Contacted','Engaged','Qualified','Converted','On Hold','Unresponsive','Disqualified','Lost']
const FILTER_KEYS=['q','status','temperature','owner_id','region','followup','sort','dir']

// Column header with a sort + filter menu. `options` are [{value,label}]; omit for sort-only columns.
function ColumnMenu({label,sortKey,sort,dir,onSort,filterValue='',options,onFilter,sortLabels=['Sort A → Z','Sort Z → A']}){
 ...
 return <div className="col-menu" ref={ref}>
  ...
  {open&&<div className="col-menu-pop" role="menu" style={{top:open.top,left:open.left}} onClick={e=>e.stopPropagation()}>
   ...
   {options&&<>
    <div className="col-menu-sep">Filter</div>
...
export default function ProspectsPage({route,onToast}){
 ...
 const columns=[
  ...
  {key:'temperature',label:'Signal',header:menu('Signal','temperature',{sortLabels:['Hot → Cold','Cold → Hot'],filterValue:filters.temperature,onFilter:filterBy('temperature'),options:['Hot','Warm','Cold'].map(x=>({value:x,label:x}))}),render:r=><Pill tone={temperatureTone(r.temperature)}>{r.temperature}</Pill>},
  {key:'status',label:'Status',header:menu('Status','status',{filterValue:filters.status,onFilter:filterBy('status'),options:STATUSES.map(x=>({value:x,label:x}))}),render:r=><Pill tone="neutral">{r.status}</Pill>},
  {key:'owner_name',label:'Owner',header:menu('Owner','owner_name',{filterValue:filters.owner_id,onFilter:filterBy('owner_id'),options:(facets.owners||[]).map(o=>({value:o.id,label:o.name}))})},
  {key:'next_follow_up',label:'Next follow-up',header:menu('Next follow-up','next_follow_up',{sortLabels:['Earliest first','Latest first'],filterValue:filters.followup,onFilter:filterBy('followup'),options:[{value:'overdue',label:'Overdue'},{value:'today',label:'Due today'},{value:'week',label:'Next 7 days'},{value:'none',label:'No date set'}]}),render:r=><span className={r.next_follow_up&&r.next_follow_up<new Date().toISOString().slice(0,10)?'text-danger':''}>{dateText(r.next_follow_up)}</span>},
  {key:'region',label:'Region',header:menu('Region','region',{filterValue:filters.region,onFilter:filterBy('region'),options:[...(facets.regions||[]).map(x=>({value:x,label:x})),{value:'__none__',label:'No region'}]})},...(admin?[{key:'_actions',label:'Actions',render:r=><div className="row-actions" onClick={e=>e.stopPropagation()}><button className="icon-btn" title="Edit" onClick={e=>openEdit(e,r)}><Icon name="edit" size={16}/></button><button className="icon-btn text-danger" title="Delete" onClick={e=>deleteLead(e,r)}><Icon name="trash" size={16}/></button></div>}]:[])]
