...
export default function ProspectsPage({route,onToast}){
 ...
 const columns=[
  ...
  {key:'region',label:'Region',header:menu('Region',null,{filterValue:filters.region,onFilter:filterBy('region'),options:[...(facets.regions||[]).map(x=>({value:x,label:x})),...(facets.has_empty_region?[{value:'__none__',label:'No region'}]:[])]})},...(admin?[{key:'_actions',label:'Actions',render:r=><div className="row-actions" onClick={e=>e.stopPropagation()}><button className="icon-btn" title="Edit" onClick={e=>openEdit(e,r)}><Icon name="edit" size={16}/></button><button className="icon-btn text-danger" title="Delete" onClick={e=>deleteLead(e,r)}><Icon name="trash" size={16}/></button></div>}]:[])]
