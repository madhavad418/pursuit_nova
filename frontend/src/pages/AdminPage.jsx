...
export default function AdminPage({onToast}){
 ...
 const openEditUser=r=>{setUserForm({id:r.id,name:r.name,role:r.role,manager_id:r.manager_id||'',title:r.title||'',region:r.region||'',category:r.category||'',active:r.active,password:''});setUserModal('edit')}
 const saveUser=async e=>{e.preventDefault();const {id,...f}=userForm;const manager_id=f.manager_id?Number(f.manager_id):null
  try{
   ...
   else{const original=users.find(x=>x.id===id);const payload={manager_id,title:f.title,region:f.region,category:f.category,active:f.active};if(f.role!==original?.role)payload.role=f.role;if(f.password)payload.password=f.password;await api.put(`/api/users/${id}`,payload);onToast?.({message:'User updated'})}
