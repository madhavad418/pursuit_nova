...
function MultiRegionSelect({value,onChange}){
 const selected=(value||'').split(',').map(s=>s.trim()).filter(Boolean)
 const [open,setOpen]=useState(false); const [q,setQ]=useState('')
 const filtered=REGIONS.filter(r=>!selected.includes(r)&&r.toLowerCase().includes(q.toLowerCase())).slice(0,8)
 const add=r=>{const next=[...selected,r];onChange(next.join(', '));setQ('')}
...
export default function AdminPage({onToast}){
 ...
 const saveUser=async e=>{e.preventDefault();const {id,...f}=userForm;const manager_id=f.manager_id?Number(f.manager_id):null
  try{
   ...
   else{const original=users.find(x=>x.id===id);const payload={name:f.name,email:f.email,manager_id,title:f.title,region:f.region,category:f.category,active:f.active};if(f.role!==original?.role)payload.role=f.role;if(f.password)payload.password=f.password;await api.put(`/api/users/${id}`,payload);onToast?.({message:'User updated'})}
 ...
 <Modal open={!!userModal} onClose={()=>setUserModal(null)} title={userModal==='create'?'Add user':'Update user'} eyebrow={userModal==='edit'?userForm.name:'New team member'}><form onSubmit={saveUser}>
  <div className="form-grid"><Field label="Full name" required><Input required value={userForm.name||''} onChange={e=>setU('name',e.target.value)}/></Field><Field label="Work email" required><Input required type="email" value={userForm.email||''} onChange={e=>setU('email',e.target.value)}/></Field>{userModal==='create'&&<Field label="Temporary password" required><Input required type="password" minLength={8} value={userForm.password} onChange={e=>setU('password',e.target.value)}/></Field>}</div>
  <div className="form-grid"><Field label="Role" required><Select required value={userForm.role} onChange={e=>setU('role',e.target.value)}>{roleOptions.map(r=><option key={r.name} value={r.name} disabled={r.locked}>{r.name}{r.locked?' (current)':''}</option>)}</Select></Field><Field label="Reporting manager" required={!isSuper}><Select value={userForm.manager_id||''} onChange={e=>setU('manager_id',e.target.value)}>{isSuper&&<option value="">No manager</option>}{users.filter(x=>x.id!==userForm.id).map(x=><option key={x.id} value={x.id}>{x.name} · {x.role}</option>)}</Select></Field><Field label="Title"><Input value={userForm.title||''} onChange={e=>setU('title',e.target.value)}/></Field><Field label="Region"><MultiRegionSelect value={userForm.region||''} onChange={v=>setU('region',v)}/></Field><Field label="Category"><Select value={userForm.category||''} onChange={e=>setU('category',e.target.value)}><option value="">Select category</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</Select></Field>{userModal==='edit'&&<Field label="Status"><Select value={userForm.active?'1':'0'} onChange={e=>setU('active',e.target.value==='1')}><option value="1">Active</option><option value="0">Inactive</option></Select></Field>}{userModal==='edit'&&<Field label="Set new password" hint="Leave empty to keep their current password. Minimum 8 characters."><Input type="password" minLength={8} autoComplete="new-password" value={userForm.password||''} onChange={e=>setU('password',e.target.value)}/></Field>}</div>
