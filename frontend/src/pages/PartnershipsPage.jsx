...
import { Button,Empty,ErrorBanner,Field,Input,Modal,Pagination,Pill,SectionHeader,Select,Table,TagsInput,Textarea,temperatureTone } from '../components/UI'
import { Icon } from '../components/Icons'
import { navigate } from '../lib/router'
import { useAuth } from '../components/Auth'
import { PartnershipEditor } from '../components/PartnershipEditor'
import { REGIONS,LEAD_SOURCES,VERTICALS } from '../components/ProspectEditor'

// Mirrors ProspectsPage.jsx exactly — same list, filters, columns, Create form and CSV import —
// pointed at the partnership endpoints so nothing here can touch a prospect's data.
...
export default function PartnershipsPage({route,onToast}){
 ...
 const load=async()=>{setError(null);try{const [d,v]=await Promise.all([api.get('/api/query/partnerships?'+qs),api.get('/api/saved-views?module=partnerships')]);setData(d);setViews(v)}catch(e){setError(e)}}
 useEffect(()=>{load()},[qs]); useEffect(()=>{api.get('/api/users/assignable').then(setUsers).catch(()=>{})},[])
 useEffect(()=>{if(form.mode==='existing'&&form.company_name.trim().length>=2){const t=setTimeout(()=>api.get('/api/query/partner-companies?q='+encodeURIComponent(form.company_name)+'&page_size=8').then(r=>setCompanyHits(r.items||[])).catch(()=>setCompanyHits([])),250);return()=>clearTimeout(t)}setCompanyHits([])},[form.mode,form.company_name])
 const set=(k,v)=>setForm(f=>({...f,[k]:v}))
 const create=async e=>{e.preventDefault();setSaving(true);try{const payload={lead:{owner_id:Number(form.owner_id||user.id),temperature:form.temperature,source:form.source,source_detail:form.source_detail,region:form.region,country:form.country,state:form.state,city:form.city,next_follow_up:form.next_follow_up||null,remarks:form.remarks},contacts:form.contact_name?[{name:form.contact_name,designation:form.designation,email:form.email,phone:form.phone,linkedin_url:form.contact_linkedin,is_primary:true}]:[]}; if(form.mode==='existing')payload.company_id=Number(form.company_id); else payload.company={name:form.company_name,vertical:form.vertical,website:form.website,linkedin_url:form.linkedin_url,region:form.region,country:form.country,state:form.state,city:form.city}; const r=await api.post('/api/partnerships/full',payload);onToast?.({message:`${r.lead.company_name} added to PursuitNova`});setOpen(false);setForm({...emptyForm,owner_id:String(user.id)});await load();navigate(`partnership/${r.lead.id}`)}catch(e){onToast?.({type:'error',message:e.message})}finally{setSaving(false)}}
 const saveView=async()=>{if(!viewName.trim())return;await api.post('/api/saved-views',{module:'partnerships',name:viewName.trim(),filters:Object.fromEntries(FILTER_KEYS.map(k=>[k,filters[k]]))});setViewOpen(false);setViewName('');load();onToast?.({message:'Saved view created'})}
 ...
 const doImport=async()=>{
  ...
  try{
   ...
   const data=await api.raw('/api/partnerships/import',{method:'POST',body:form})
   setImportResult(data);if(data.imported>0)load()
   onToast?.({message:`${data.imported} partnership(s) imported`})
  }catch(err){setImportResult({imported:0,skipped:0,errors:[{row:0,message:err.message}],warnings:[]})}finally{setImporting(false)}
 }
 const downloadSample=()=>{window.open('/api/partnerships/import/sample','_blank')}
 const [editId,setEditId]=useState(null)
 const openEdit=(e,r)=>{e.stopPropagation();setEditId(r.id)}
 const deletePartnership=async(e,r)=>{e.stopPropagation();if(!confirm(`Delete partnership "${r.company_name}"? This will also remove all related meetings, actions and opportunities.`))return;try{await api.delete(`/api/partnerships/${r.id}`);onToast?.({message:`${r.company_name} deleted`});load()}catch(err){onToast?.({type:'error',message:err.message})}}
 ...
 const columns=[
  {key:'company_name',label:'Partner',header:menu('Partner','company_name'),render:r=><div className="primary-cell"><strong>{r.company_name}</strong><span>{r.primary_contact||'No primary contact'} · {r.vertical}</span></div>},
  ...
  {key:'region',label:'Region',header:menu('Region',null,{filterValue:filters.region,onFilter:filterBy('region'),options:[...(facets.regions||[]).map(x=>({value:x,label:x})),...(facets.has_empty_region?[{value:'__none__',label:'No region'}]:[])]})},...(admin||has('LEAD_EDIT')||has('COMPANY_EDIT')?[{key:'_actions',label:'Actions',render:r=><div className="row-actions" onClick={e=>e.stopPropagation()}>{(r.can_edit||admin)&&<button className="icon-btn" title="Edit" aria-label={`Edit ${r.company_name}`} onClick={e=>openEdit(e,r)}><Icon name="edit" size={16}/></button>}{admin&&<button className="icon-btn text-danger" title="Delete" aria-label={`Delete ${r.company_name}`} onClick={e=>deletePartnership(e,r)}><Icon name="trash" size={16}/></button>}</div>}]:[])]
 return <><SectionHeader eyebrow="Alliance workspace" title="Partnerships" text="Own the relationship, next action and commercial signal with every channel and technology partner." actions={<><div className="saved-view-menu"><Select value="" onChange={e=>{const v=views.find(x=>String(x.id)===e.target.value);if(v)applyView(v)}}><option value="">Saved views ({views.length})</option>{views.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</Select></div><Button variant="soft" icon="filter" onClick={()=>setViewOpen(true)}>Save view</Button>{has('LEAD_CREATE')&&<><Button variant="soft" onClick={()=>{setImportOpen(true);setImportFile(null);setImportResult(null)}}>Import CSV</Button><Button icon="plus" onClick={()=>setOpen(true)}>New partnership</Button></>}</>}/><div className="mini-kpis"><div><span>Visible partnerships</span><strong>{data?.total??'—'}</strong></div><div><span>Hot</span><strong className="text-danger">{data?.summary?.hot??'—'}</strong></div><div><span>Warm</span><strong className="text-warning">{data?.summary?.warm??'—'}</strong></div><div><span>Follow-ups overdue</span><strong>{data?.summary?.overdue_followups??'—'}</strong></div></div><div className="toolbar-card"><div className="search-input"><Icon name="search"/><Input placeholder="Search partner, owner or remarks" value={filters.q} onChange={e=>setFilters(f=>({...f,q:e.target.value,page:1}))}/></div><Select value={filters.temperature} onChange={e=>setFilters(f=>({...f,temperature:e.target.value,page:1}))}><option value="">All signals</option><option>Hot</option><option>Warm</option><option>Cold</option></Select><Select value={filters.status} onChange={e=>setFilters(f=>({...f,status:e.target.value,page:1}))}><option value="">All statuses</option>{['New','Assigned','Contacted','Engaged','Qualified','Converted','On Hold','Unresponsive','Disqualified','Lost'].map(x=><option key={x}>{x}</option>)}</Select><button className="icon-btn" onClick={load}><Icon name="refresh"/></button></div><ErrorBanner error={error} onRetry={load}/>{data&&<section className="panel table-panel">{data.items?.length?<><Table columns={columns} rows={data.items} onRowClick={r=>navigate(`partnership/${r.id}`)}/><Pagination page={data.page} pages={data.pages} onPage={page=>setFilters(f=>({...f,page}))}/></>:<Empty title="No partnerships in this view" text="Change the filters or create the first partnership."/>}</section>}<Modal open={viewOpen} onClose={()=>setViewOpen(false)} title="Save current view" eyebrow="Personal productivity"><Field label="View name" required><Input autoFocus value={viewName} onChange={e=>setViewName(e.target.value)} placeholder="e.g. My hot partnerships"/></Field><div className="filter-summary"><span>Temperature: <b>{filters.temperature||'Any'}</b></span><span>Status: <b>{filters.status||'Any'}</b></span><span>Search: <b>{filters.q||'None'}</b></span></div><div className="modal-actions"><Button variant="ghost" onClick={()=>setViewOpen(false)}>Cancel</Button><Button onClick={saveView}>Save view</Button></div></Modal><Modal open={open} onClose={()=>setOpen(false)} title="Create partnership" eyebrow="New alliance" size="xl"><form onSubmit={create}><div className="segmented"><button type="button" className={form.mode==='new'?'active':''} onClick={()=>set('mode','new')}>New company</button><button type="button" className={form.mode==='existing'?'active':''} onClick={()=>set('mode','existing')}>Existing company</button></div><div className="form-section"><h3>Company</h3><div className="form-grid">{form.mode==='new'?<><Field label="Company name" required><Input required value={form.company_name} onChange={e=>set('company_name',e.target.value)}/></Field><Field label="Vertical" required hint="Add as many as apply — pick from the list or type new ones"><TagsInput required options={VERTICALS} value={form.vertical} onChange={v=>set('vertical',v)} placeholder="e.g. Systems Integration"/></Field><Field label="Website"><Input value={form.website} onChange={e=>set('website',e.target.value)} placeholder="https://"/></Field><Field label="LinkedIn company"><Input value={form.linkedin_url} onChange={e=>set('linkedin_url',e.target.value)} placeholder="https://linkedin.com/company/..."/></Field></>:<div className="company-lookup span-2"><Field label="Find existing company" required><Input value={form.company_name} onChange={e=>{set('company_name',e.target.value);set('company_id','')}} placeholder="Type company name…"/></Field>{companyHits.length>0&&<div className="lookup-results">{companyHits.map(c=><button type="button" key={c.id} className={String(form.company_id)===String(c.id)?'selected':''} onClick={()=>{set('company_id',String(c.id));set('company_name',c.name);setCompanyHits([])}}><strong>{c.name}</strong><span>{c.vertical} · {c.region||'No region'}</span></button>)}</div>}</div>}<Field label="Region"><Select value={form.region} onChange={e=>set('region',e.target.value)}><option value="">Select region</option>{REGIONS.map(x=><option key={x}>{x}</option>)}</Select></Field><Field label="Country"><Input value={form.country} onChange={e=>set('country',e.target.value)}/></Field><Field label="State"><Input value={form.state} onChange={e=>set('state',e.target.value)}/></Field><Field label="City"><Input value={form.city} onChange={e=>set('city',e.target.value)}/></Field></div></div><div className="form-section"><h3>Primary contact</h3><div className="form-grid"><Field label="Name"><Input value={form.contact_name} onChange={e=>set('contact_name',e.target.value)}/></Field><Field label="Designation"><Input value={form.designation} onChange={e=>set('designation',e.target.value)}/></Field><Field label="Email"><Input type="email" value={form.email} onChange={e=>set('email',e.target.value)}/></Field><Field label="Phone"><Input value={form.phone} onChange={e=>set('phone',e.target.value)}/></Field><Field label="LinkedIn"><Input value={form.contact_linkedin} onChange={e=>set('contact_linkedin',e.target.value)}/></Field></div></div><div className="form-section"><h3>Partnership ownership</h3><div className="form-grid"><Field label="Owner"><Select value={form.owner_id} onChange={e=>set('owner_id',e.target.value)}>{users.map(x=><option key={x.id} value={x.id}>{x.name} · {x.role}</option>)}</Select></Field><Field label="Signal"><Select value={form.temperature} onChange={e=>set('temperature',e.target.value)}><option>Hot</option><option>Warm</option><option>Cold</option></Select></Field><Field label="Source"><Select value={form.source} onChange={e=>set('source',e.target.value)}>{LEAD_SOURCES.map(x=><option key={x}>{x}</option>)}</Select></Field><Field label="Source detail"><Input value={form.source_detail} onChange={e=>set('source_detail',e.target.value)}/></Field><Field label="Next follow-up"><Input type="date" value={form.next_follow_up} onChange={e=>set('next_follow_up',e.target.value)}/></Field><Field label="Remarks" className="span-2"><Textarea value={form.remarks} onChange={e=>set('remarks',e.target.value)}/></Field></div></div><div className="modal-actions"><Button type="button" variant="ghost" onClick={()=>setOpen(false)}>Cancel</Button><Button type="submit" disabled={saving}>{saving?'Creating…':'Create partnership'}</Button></div></form></Modal>
<PartnershipEditor partnershipId={editId} open={!!editId} onClose={()=>setEditId(null)} onSaved={load} onToast={onToast}/>
<Modal open={importOpen} onClose={()=>setImportOpen(false)} title="Import partnerships from CSV" eyebrow="Bulk import" size="lg">
 <div className="import-section">
  <p style={{fontSize:13,color:'var(--muted)',margin:'0 0 12px'}}>Upload a CSV file to bulk-import partnerships. Existing companies will be linked, not duplicated.</p>
